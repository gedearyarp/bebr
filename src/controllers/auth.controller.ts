...
const generateTokens = (user: { id: string; username: string; email: string }) => {
  ...
  const options: SignOptions = { expiresIn: (jwtExpiresIn || '1h') as jwt.SignOptions['expiresIn'] };
  const refreshOptions: SignOptions = { expiresIn: (jwtRefreshExpiresIn || '7d') as jwt.SignOptions['expiresIn'] };
